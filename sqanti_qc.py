...
def reference_parser(args, chrom_names):
	...
	## gtf to genePred
 	if args.name:
 		subprocess.call(["gtfToGenePred", args.annotation, referenceFiles, '-genePredExt', '-allErrors', '-ignoreGroupsWithoutExons', '-geneNameAsName2'])
 	else:
 		subprocess.call(["gtfToGenePred", args.annotation, referenceFiles, '-genePredExt', '-allErrors', '-ignoreGroupsWithoutExons'])
...
def isoforms_parser(args):
	...
	global queryFile
 	...
 	# gtf to genePred
 	try:
 		subprocess.call(["gtfToGenePred", corrGTF, queryFile , '-genePredExt', '-allErrors', '-ignoreGroupsWithoutExons'])
	except (RuntimeError, TypeError, NameError):
